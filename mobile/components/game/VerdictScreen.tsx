...
function PulsingGlow() {
  ...
  return (
    <Animated.View
      style={[glowStyles.ring, { transform: [{ scale }], opacity }]}
    />
  );
}
...
export default function VerdictScreen({
  ...
  const optionOpacities = useRef(
    suspects.map(() => new Animated.Value(0)),
  ).current;
  const optionSlides = useRef(
    suspects.map(() => new Animated.Value(16)),
  ).current;
  ...
  useEffect(() => {
    ...
    suspects.forEach((_, i) => {
      Animated.parallel([
        Animated.timing(optionOpacities[i], {
          toValue: 1,
          duration: 350,
          delay: 200 + i * 80,
          useNativeDriver: true,
        }),
        Animated.spring(optionSlides[i], {
          toValue: 0,
          delay: 200 + i * 80,
          useNativeDriver: true,
          tension: 70,
  ...
  return (
    <View style={styles.root}>
      <LinearGradient
        colors={["#0A0D1A", colors.bg, colors.bg]}
      ...
      >
        ...
        <Text style={styles.subtitle}>
          You have examined the evidence, heard the witnesses, studied the
          timeline, and analysed every suspect.{"\n\n"}
          {isSurvival
        ...
        {isSurvival && (
          <View style={styles.survivalWarning}>
            <View
              style={[styles.swBorder, { borderColor: "rgba(155,89,182,0.4)" }]}
            />
            <Text style={styles.swText}>
              One shot. One verdict. No second chances.
            </Text>
          </View>
        )}
        ...
        <View style={styles.options}>
          {suspects.map((name, idx) => {
            ...
            const wrong =
              submitted && name === selected && name !== correctSuspect;
            ...
            const textColor = correct
              ...
              : wrong
                ...
                : isSelected
                  ? colors.gold
                  : colors.text;
            const icon: keyof typeof Feather.glyphMap = correct
              ? "check-circle"
              : wrong
                ? "x-circle"
                : isSelected
            ...
            return (
              <Animated.View
                ...
                style={{
                  ...
                  transform: [{ translateY: optionSlides[idx] }],
                }}
              >
                <Pressable
                  onPress={() => select(name)}
        ...
        {(isCorrectAnswer || isWrongAnswer) && (
          ...
          >
            ...
            >
              ...
              />
              <Feather
                name={isCorrectAnswer ? "award" : "alert-triangle"}
                size={24}
