...
import {
  RankIcon, IconArrowLeft, IconSettings, IconVibration, IconSparkles, IconSun, IconBook,
  IconWarning, IconFolder, IconTrendingUp, IconCoin, IconFire, IconTrash,
...
function ToggleRow({ icon, label, sub, value, onChange, disabled }: { icon: React.ReactNode; label: string; sub: string; value: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    ...
    <View style={[styles.root, { paddingTop: topPad }]}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}
        ...
        <LinearGradient colors={[rank.gradTop, rank.gradBot]} style={styles.playerCard}>
          ...
          <RankIcon id={rank.svgIcon} size={40} color={rank.color} />
          <View>
        ...
        <SectionHead icon={<IconSettings size={16} color={colors.textMuted} />} title="Detective Identity" />
        <LinearGradient colors={[colors.surface2, colors.surface1]} style={styles.section}>
          ...
          <View style={styles.row}>
            <View style={styles.rowLeft}>
              <View style={styles.rowIcon}><RankIcon id="rookie" size={22} color={colors.textMuted} /></View>
              <View>
        ...
        <SectionHead icon={<IconSettings size={16} color={colors.textMuted} />} title="Gameplay" />
        <LinearGradient colors={[colors.surface2, colors.surface1]} style={styles.section}>
          <View style={[styles.sectionBorder, { borderColor: colors.border }]} />
          <ToggleRow icon={<IconVibration size={22} color={colors.textMuted} />} label="Haptic Feedback" sub="Vibrations on correct/wrong answers" value={haptics} onChange={setHaptics} />
          <View style={styles.divider} />
          <ToggleRow icon={<IconSparkles size={22} color={colors.textMuted} />} label="Animations" sub="Entrance effects and transitions" value={animations} onChange={setAnimations} />
          <View style={styles.divider} />
          <ToggleRow icon={<IconSun size={22} color={colors.textMuted} />} label="Dark Mode" sub="Always on — it's a detective game" value={darkMode} onChange={() => {}} disabled />
        </LinearGradient>

        <SectionHead icon={<IconTrendingUp size={16} color={colors.textMuted} />} title="Your Progress" />
        <LinearGradient colors={[colors.surface2, colors.surface1]} style={styles.section}>
          <View style={[styles.sectionBorder, { borderColor: colors.border }]} />
          {[
            { icon: <IconFolder size={20} color={colors.gold} />, label: "Cases Solved", val: `${game.solvedCases.length} / 6` },
            { icon: <IconTrendingUp size={20} color={colors.gold} />, label: "Total XP Earned", val: `${game.totalXPEarned} XP` },
            { icon: <IconCoin size={20} color={colors.gold} />, label: "Coins", val: String(game.coins) },
            { icon: <IconFire size={20} color={colors.gold} />, label: "Current Streak", val: `${game.streak} day${game.streak !== 1 ? "s" : ""}` },
            { icon: <IconSettings size={20} color={colors.gold} />, label: "Games Played", val: String(game.gamesPlayed) },
          ].map((s, i, arr) => (
            <View key={s.label}>
        ...
        <SectionHead icon={<IconBook size={16} color={colors.textMuted} />} title="About" />
        <LinearGradient colors={[colors.surface2, colors.surface1]} style={styles.section}>
          <View style={[styles.sectionBorder, { borderColor: colors.border }]} />
        ...
        <SectionHead icon={<IconWarning size={16} color={colors.red} />} title="Danger Zone" />
...
const styles = StyleSheet.create({
  ...
  statVal: { fontFamily: "Inter_700Bold", fontSize: 14, color: colors.gold, marginLeft: "auto" as any },
  aboutBox: { gap: 8 },
  ...
  resetBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: colors.radius.md, padding: 14, borderWidth: 1, borderColor: "rgba(232,64,64,0.35)" },
