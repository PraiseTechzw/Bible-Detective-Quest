  ...
  return (
    <View style={taStyles.wrap}>
      ...
      <View style={taStyles.row}>
        ...
        <Text style={[taStyles.time, { color: barColor }]}>
  ...
  return (
    <View style={svStyles.wrap}>
      {Array.from({ length: 3 }).map((_, i) => (
        <View
          key={i}
          style={[svStyles.heart, { opacity: i < lives ? 1 : 0.2 }]}
        >
...
function GameOverScreen({
  ...
  return (
    <View style={goStyles.root}>
      <LinearGradient
        colors={["#1A0808", "#0D0404", colors.bg]}
      ...
      >
        <LinearGradient
          colors={["rgba(232,64,64,0.2)", "rgba(232,64,64,0.05)"]}
          style={goStyles.iconBg}
        >
          <IconWarning size={44} color={colors.red} />
        </LinearGradient>
        <Text style={goStyles.title}>
          {isTimeout ? "Time's Up!" : "Case Closed — By Mistake"}
        </Text>
        ...
        <View style={goStyles.btnRow}>
          <Animated.View style={{ flex: 1 }}>
            <Text onPress={onRetry} style={goStyles.retryBtn}>
              Try Again
            </Text>
          </Animated.View>
          <Animated.View style={{ flex: 1 }}>
            <Text onPress={onExit} style={goStyles.exitBtn}>
              Exit
            </Text>
          </Animated.View>
        </View>
...
export default function CaseScreen() {
  ...
  const difficultySettings = caseData
    ? getDiffSettings(caseData.difficulty, level)
    : getDiffSettings("Beginner", level);

  const [step, setStep] = useState<GameStep>("intro");
  const [timeLeft, setTimeLeft] = useState(difficultySettings.timeSeconds);
  const [survivalLives, setSurvivalLives] = useState(difficultySettings.lives);
  ...
  const penalizeTime = () => {
    if (gameMode === "timeAttack") {
      setTimeLeft((t) => Math.max(0, t - difficultySettings.penalty));
    }
  ...
  const handleReveal = () => {
    ...
    if (!isCaseSolved(caseData.id)) {
      const elapsed =
        Math.max(0, difficultySettings.timeSeconds - timeLeft) * 1000;
  ...
  const handleRetry = () => {
    setStep("intro");
    setTimeLeft(difficultySettings.timeSeconds);
    setSurvivalLives(difficultySettings.lives);
    setIsGameOver(false);
  };
    ...
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      ...
      {step !== "reveal" && gameMode === "timeAttack" && (
        <TimeAttackBar
          ...
          total={difficultySettings.timeSeconds}
        />
      )}
